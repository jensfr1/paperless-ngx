...
class RemoteDocumentParser:
    ...
    def _build_searchable_pdf(
        ...
        opened: list = []
        try:
            for index, (image, page_dpi) in enumerate(images):
                ...
                # The invisible text layer is rendered over the page raster.
                # Rasterising is intentional: it mirrors how OCR archives are
                # produced for image documents and avoids fragile mapping
                # between the source pixel space and PDF point space.
...
def _load_page_images(source: Path, mime_type: str, dpi: int) -> list[tuple]:
    """Return ``(RGB PIL.Image, dpi)`` for each page of the source document.

    PDF input is rasterised with pdf2image at ``dpi``; image input (including
    multi-frame TIFF) is loaded directly, one entry per frame, honouring the
    image's embedded DPI when present so the archive page keeps its physical
    size (falling back to ``dpi`` otherwise).
    """
    ...
    if mime_type == "application/pdf":
        ...
        return [(image, float(dpi)) for image in convert_from_path(source, dpi=dpi)]
    ...
    with Image.open(source) as opened:
        for frame in ImageSequence.Iterator(opened):
            ...
            pages.append((frame.convert("RGB"), page_dpi))
    return pages
